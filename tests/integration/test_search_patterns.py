...
class TestSearchPatterns:
    ...
    def test_search_setup(self, model):
        ...
        parent_constraint = list(model.find(parent=parent['uuid'], attribute_2='e_15_attribute_2'))
        assert len(parent_constraint) == 1
        assert parent_constraint[0]['name'] == 'entity_15'
        
        # 5. Test combination of patterns
        combination = list(model.find(
            parent=parent['uuid'], 
        ...
        # Log performance
        logging.info(f"Complex search found {len(results)} results in {duration:.6f} seconds") 